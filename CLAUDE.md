...
MVP + 이후 요구사항(언어 선택, 다중 선택) 전부 구현 완료, QA 버그 수정까지 반영됨 (2026-07-31 야간 라운드 종료 시점). Next.js(App Router, TypeScript, Tailwind) 프로젝트로 DeepL/MyMemory/Gemini/Groq/OpenRouter 5개 번역 엔진 실제 연동 + 노션 저장까지 동작 확인함.

- `PRD.md` — 번역 비교 앱의 제품 요구사항 문서. 목표, 기능 범위, 기술 스택, 성공 기준, §15 "다음 우선순위 백로그"까지 정리되어 있음. 새 작업 시작 전 먼저 읽을 것.
...
- `app/api/notion/route.ts` — 선택한 번역(다중 선택 가능) + 엔진별 결과를 노션 데이터베이스에 저장하는 API Route. `selectedEngineIds`를 화이트리스트로 검증 후 Multi-select에 저장.
...
- OpenRouter 무료(`:free`) 모델 목록은 자주 바뀐다. 현재 로테이션 목록(`lib/engines/openrouter.ts`의 `FREE_MODELS`)은 `inclusionai/ling-3.0-flash:free`, `nvidia/nemotron-3-nano-30b-a3b:free`, `google/gemma-4-31b-it:free`로 구성됨. **2026-07-31 재점검 완료**: `https://openrouter.ai/api/v1/models` 실제 호출 결과 이 3개가 모두 여전히 `:free` 목록에 존재함을 확인(당시 전체 무료 모델 14개 중). 3개 모두 직접 chat/completions 호출로 정상 응답 확인(1순위 `ling-3.0-flash`는 `/api/translate`에 `enabledEngines:["openrouter"]`로 실제 호출해 "The quick brown fox..." → "빠른 갈색 여우가 게으른 개를 뛰어넘는다."로 정상 번역 확인; 2·3순위 `nemotron-3-nano-30b-a3b`/`gemma-4-31b-it`도 직접 호출로 200 응답 확인). 교체 불필요, 목록 유지. 크기도 각각 124B/30B/30.7B급이라 너무 작은 모델은 아님. 다음에 다시 점검할 때 이 3개 중 하나라도 목록에서 사라지면 `https://openrouter.ai/api/v1/models` 조회 후 `:free` 접미사이면서 파라미터 규모가 지나치게 작지 않은 모델로 교체할 것.
...
- 노션 저장에 중복 방지 가드가 없다. 같은 내용으로 "노션에 저장"을 여러 번 누르면 중복 행이 그대로 생성된다 (`PRD.md` §15 백로그 1순위).
...
- 새 기능을 구현하기 전에 반드시 `PRD.md`를 먼저 읽는다. 특히 §15 "다음 우선순위 백로그"에 다음 라운드 후보가 정리돼 있다.
...
- 노션 저장은 **수동 버튼 클릭으로만** 발생해야 한다 (자동 저장 금지). 저장 항목은 **다중 선택된 번역기들**이며, 그 순간 호출됐던 모든 엔진의 결과도 함께 백업된다 (`PRD.md` §6.3, §8). 단일 선택으로 되돌리지 말 것.
...
- Part 2의 특색 아이디어(①~⑥, `PRD.md` §7)는 이번 목적과 직접 관련이 적어 우선순위가 낮다. 시간 여유가 있을 때만 1~2개 선택해 얹는다(§15 백로그에서 ①번을 추천해둠).
