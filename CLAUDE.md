...
- `app/api/konglish/route.ts`, `lib/konglish.ts`, `components/tabs/KonglishTab.tsx` (2026-08-05) — "오타 변환기" 옆의 독립 유틸리티 탭(콩글리시 찾기). "키친/원피스/핸드폰"처럼 한국식으로 굳어진 영어 표현의 실제 영어 단어를 네이버/구글 검색 없이 앱 안에서 바로 찾게 해달라는 요청으로 추가. `lib/engines/prompt.ts`의 `buildKonglishPrompt` 프롬프트를 1순위 Gemini(`findRealEnglish`)로 호출하고, 실패 시 Groq(`findRealEnglishWithGroq`)로 폴백(OCR과 같은 우선순위 폴백 패턴, 다만 이미지가 아니라 텍스트라 OpenRouter까지는 안 씀). 새 API 없이 기존 Gemini/Groq 키만 재사용.
